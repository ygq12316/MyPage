    ...
    <div class="comment-input-wrapper">
      <div style="display:flex">
        <v-avatar size="40">
          ...
          <img
            v-else
            src="https://big-event20040810.oss-cn-beijing.aliyuncs.com/avatar/default.png"
          />
        </v-avatar>
...
export default {
  ...
  methods: {
    replyComment(index, item) {
      ...
      // 设置回复框信息
      if (this.$refs.reply && this.$refs.reply[index]) {
        const replyComponent = this.$refs.reply[index];
        replyComponent.commentContent = "";
        replyComponent.nickname = item.nickname;
        replyComponent.replyId = item.userId;
        replyComponent.parentId = this.commentList[index].id;
        replyComponent.chooseEmoji = false;
        replyComponent.index = index;
        replyComponent.showReply();

        // 让回复框中的文本域自动获得焦点
        this.$nextTick(() => {
          const textarea = replyComponent.$el.querySelector("textarea");
          if (textarea) {
            textarea.focus();
          }
        });
    ...
    insertComment() {
      ...
      if (!this.$store.state.userId) {
        ...
        this.$toast({ type: "warning", message: "请先登录后再发表评论" });
        return false;
      }
      ...
      var comment = {
        ...
        parentId: null,  // 明确设置为null表示这是顶级评论
        replyId: null    // 明确设置为null表示不是回复
      };
      ...
      this.axios.post("/api/comments", comment)
        .then(({ data }) => {
          if (data.flag) {
            ...
            this.$toast({ type: "error", message: data.message || "评论失败" });
          }
        })
        .catch(error => {
          // 检查是否是登录问题
          if (error.response && error.response.status === 401) {
            this.$store.state.loginFlag = true;
            this.$toast({ type: "warning", message: "登录已过期，请重新登录" });
          } else {
            this.$toast({ type: "error", message: "评论失败，请稍后重试" });
          }
        });
    },
    like(comment) {
